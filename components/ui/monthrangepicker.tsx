...
function MonthRangePicker({
    ...
    return (
        <div className={cn("min-w-[400px] border border-border p-3 mt-4 bg-background rounded-lg shadow-sm", className)} {...props}>
        ...
        <div className="flex gap-4 bg-background">
            <div className="min-w-[400px] space-y-4">
                <div className="flex justify-evenly pt-1 relative items-center">
                    <div className="text-sm font-medium text-foreground">{callbacks?.yearLabel ? callbacks?.yearLabel(menuYear) : menuYear}</div>
                    ...
                    <div className="text-sm font-medium text-foreground">{callbacks?.yearLabel ? callbacks?.yearLabel(menuYear + 1) : menuYear + 1}</div>
                </div>
                        ...
                        {MONTHS.map((monthRow, a) => {
                            return (
                                <tr key={"row-" + a} className="flex w-full mt-2">
                                    {monthRow.map((m, i) => {
                                        const isInRange = (menuYear + m.yearOffset > startYear || (menuYear + m.yearOffset == startYear && m.number > startMonth)) &&
                                            (menuYear + m.yearOffset < endYear || (menuYear + m.yearOffset == endYear && m.number < endMonth)) &&
                                            (rangePending || endLocked);
                                            
                                        const isStartMonth = menuYear + m.yearOffset == startYear && m.number == startMonth && (rangePending || endLocked);
                                        
                                        const isEndMonth = menuYear + m.yearOffset == endYear &&
                                            m.number == endMonth &&
                                            (rangePending || endLocked) &&
                                        ...
                                        return (
                                            <td
                                                ...
                                                className={cn(
                                                    "h-10 w-1/4 text-center text-sm p-0 relative focus-within:relative focus-within:z-20",
                                                    isInRange && "bg-primary/10 text-primary-foreground",
                                                    isStartMonth && "bg-primary text-primary-foreground rounded-l-md",
                                                    isEndMonth && "bg-primary text-primary-foreground rounded-r-md",
                                                    i == 3 ? "mr-2" : i == 4 ? "ml-2" : ""
                                                )}
                                                onMouseEnter={() => {
                                            ...
                                            >
                                                <button
                                                    ...
                                                    className={cn(
                                                        ...
                                                        (isStartMonth || isEndMonth) && "hover:bg-primary hover:text-primary-foreground"
                                                    )}
                                                >
