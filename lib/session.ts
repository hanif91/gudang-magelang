...
export async function validateSessionToken(
  ...
  const [data]: any = await db.query(
    "SELECT a.id, a.userid, a.expires_at,b.email,b.nama,b.image FROM session_web_admin a INNER JOIN web_admin_user b ON b.id = a.userid WHERE a.id = ?",
    sessionId,
  );
  ...
  if (Date.now() >= session.expiresAt.getTime()) {
    await db.execute("DELETE FROM session_web_admin WHERE id = ?", [
      session.id,
    ]);
    return { session: null, user: null };
  }
