...
import { getAssetPerpipaan } from "@/lib/actions/actAssetPerpipaan"
...
export default async function edit({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  const data = await getAssetPerpipaan(id)
    ...
    <Fragment>
      <main className="flex flex-col gap-5 justify-center content-center p-5">
        <Card className="w-full">
          <CardHeader>
            <CardTitle>Edit Asset Perpipaan</CardTitle>
            <CardDescription>Edit Asset Perpipaan Form</CardDescription>
          </CardHeader>
          <CardContent className="py-0">
            <DataForm asset_perpipaan={data.data} />
          </CardContent>
