...
export async function GET(request: Request) {
	try {
      ...
      SELECT 
        ...
        u.nama AS 'nama_user',
		IF(s.qty IS NULL, 0, s.qty) AS stok,
...
// 		const { tanggal, barang_id, qty, user_id } = {
...
// 		const [rows] = await db.execute<RowDataPacket[]>('INSERT INTO dpb (nodpb,tanggal,barang_id,qty,user_id) values (noautodpb(?),?,?,?,?)', [tanggal, tanggal, barang_id, qty, user_id]);
...
export async function POST(request: NextRequest) {
	try {
		...
		const [dpbRows] = await db.execute<RowDataPacket[]>(
			"SELECT noautodpb(?) as nodpb",
			[tanggal]
		);
		const nodpb = dpbRows[0]?.nodpb;
		if (!nodpb) {
			return NextResponse.json(
				{ success: false, message: "Gagal mendapatkan nomor DPB" },
				{ status: 422 }
			);
		}
